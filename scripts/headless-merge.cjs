...
function parseDefault(field) {
    ...
    const specialValues = ['CURRENT_TIMESTAMP', 'NULL', 'TRUE', 'FALSE', 'NOW()', 'CURRENT_DATE', 'GEN_RANDOM_UUID()'];
...
function fromPostgres(sql, currentTables) {
    ...
    const tableComments = {};
    const comments = {};
    const commentRegex = /COMMENT ON (TABLE|COLUMN) (?:(?:"([^"]+)")|([\w\.]+))(?:\.(?:(?:"([^"]+)")|([\w\.]+)))? IS '(.*?)';/gi;
    let match;
    while ((match = commentRegex.exec(sql)) !== null) {
        const type = match[1].toUpperCase();
        const tName = match[2] || match[3];
    ...
    return { tables, relationships, tableComments, comments };
}
...
function generateExportSQL(diagram) {
    const tableSql = diagram.tables.map(table => {
        const fieldDefinitions = (table.fields || []).map(f => {
            ...
            const identity = f.increment ? ' GENERATED BY DEFAULT AS IDENTITY' : '';
            return `	"${f.name}" ${f.type}${sizeStr}${f.notNull ? ' NOT NULL' : ''}${f.unique ? ' UNIQUE' : ''}${identity}${dVal !== "" ? ' DEFAULT '+dVal : ''}`.replace(/\s+/g, ' ').trimEnd();
        }).join(",\n");
        ...
        result.tables.forEach(newT => {
            if (tableMap.has(newT.name)) {
                ...
                newT.fields.forEach(nf => {
                    const of = existing.fields.find(f => f.name === nf.name);
                    if (of && !nf.comment) nf.comment = of.comment;
                });
                tableMap.set(newT.name, { ...existing, fields: newT.fields, comment: newT.comment || existing.comment });
            } else {
                const maxX = Math.max(0, ...Array.from(tableMap.values()).map(t => t.x || 0)) + 150;
                newT.x = maxX; newT.y = 100; newT.position = { x: maxX, y: 100 };
                tableMap.set(newT.name, newT);
            }
        });
